...
class PolygonFlatFilesDownloader:
    ...
    def __init__(self, api_key: str, data_root: str = "./app/data/cache"):
        ...
        self.base_url = (
            "https://files.polygon.io/flatfiles/global_crypto/minute_aggs_v1"
        )
    ...
    def download_date_range(
        ...
        logger.info(
            f"Downloading {len(date_list)} files from {start_date.date()} to {end_date.date()}"
        )
        ...
        logger.info(
            f"Download complete: {stats['downloaded']} downloaded, {stats['skipped']} skipped, {stats['failed']} failed"
        )
        return stats
