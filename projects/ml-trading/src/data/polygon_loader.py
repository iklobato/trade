...
class PolygonLoader:
    ...
    def load_date_range(
        ...
        logger.info(
            f"Loaded {len(combined_df)} total records from {start_date.date()} to {end_date.date()}"
        )
        return combined_df
    ...
    def resample_data(
        self, data: pd.DataFrame, timeframe: str = "1min", agg_method: str = "ohlc"
    ) -> pd.DataFrame:
        """
        ...
        logger.info(
            f"Resampled {len(data)} records to {timeframe}: {len(resampled)} records"
        )
        return resampled
    ...
    def validate_data_quality(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "negative_prices": (data[["open", "high", "low", "close"]] <= 0)
            .any()
            .any(),
