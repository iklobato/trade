...
class PerformanceMetrics:
    ...
    def calculate_all_metrics(
        self, equity_curve: pd.DataFrame, trades: pd.DataFrame
    ) -> Dict[str, Any]:
        """
    ...
    def _calculate_return_metrics(self, equity_curve: pd.DataFrame) -> Dict[str, float]:
        ...
        annualized_return = (
            (1 + total_return) ** (1 / time_span) - 1 if time_span > 0 else 0.0
        )
    ...
    def _calculate_risk_metrics(self, equity_curve: pd.DataFrame) -> Dict[str, float]:
        ...
        # Calmar ratio
        annualized_return = self._calculate_return_metrics(equity_curve)[
            "annualized_return"
        ]
        calmar_ratio = (
    ...
    def _calculate_advanced_metrics(
        ...
        sortino_ratio = (
            np.mean(returns) / downside_deviation if downside_deviation > 0 else 0.0
        )
        ...
        trade_pnls = trades["net_pnl"].values
        sqn = (
            np.mean(trade_pnls) / np.std(trade_pnls) * np.sqrt(len(trade_pnls))
    ...
    def calculate_rolling_metrics(
        self, equity_curve: pd.DataFrame, window: int = 100
    ) -> pd.DataFrame:
        """
