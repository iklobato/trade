...
class TradeManager:
    ...
    def can_open_position(self, symbol: str, size: float, price: float) -> bool:
        ...
        if required_capital > self.capital:
            logger.warning(f"Insufficient capital for {symbol}: {required_capital:.2f} > {self.capital:.2f}")
            return False
    ...
    def open_position(self, 
                     ...
                     stop_loss: Optional[float] = None,
                     take_profit: Optional[float] = None,
                     strategy: str = "ML") -> Optional[Position]:
        """
        ...
        # Calculate costs
        costs = calculate_trading_costs(price, final_size, side, 5.0, 3.0)  # 5bps fee, 3bps slippage
        
        # Check if we still have enough capital after costs
        total_cost = costs['notional'] + costs['total_cost']
        if total_cost > self.capital:
            logger.warning(f"Insufficient capital after costs: {total_cost:.2f} > {self.capital:.2f}")
            return None
        ...
        try:
            ...
            logger.info(f"Opened {side} position: {final_size:.6f} {symbol} at {costs['effective_price']:.2f}")
    ...
    def close_position(self, position: Position, exit_price: float, reason: str = "manual") -> Optional[Trade]:
        """
        ...
        try:
            ...
            logger.info(f"Closed {position.side} position: PnL {net_pnl:.2f} ({pnl_pct:.2%}) - {reason}")
    ...
    def load_state(self, filepath: str):
        ...
        self.daily_pnl = {datetime.fromisoformat(k).date(): v for k, v in state['daily_pnl'].items()}
