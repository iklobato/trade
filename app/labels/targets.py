...
class LabelGenerator:
    ...
    def __init__(self, horizon_minutes: int = 10, up_threshold: float = 0.002, down_threshold: float = -0.002):
        self.horizon_minutes = horizon_minutes
        self.up_threshold = up_threshold
        self.down_threshold = down_threshold
        
    def generate_labels(self, data: pd.DataFrame, price_col: str = 'close') -> pd.DataFrame:
        """
        Generate classification labels based on forward returns.
    ...
    def get_label_statistics(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'class_balance_ratio': min(label_counts) / max(label_counts) if len(label_counts) > 0 else 0
        }
    ...
    def validate_labels(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if len(label_counts) > 0:
            ...
            if imbalance_ratio < 0.1:
                validation['warnings'].append(f'Extreme class imbalance: {imbalance_ratio:.3f}')
            elif imbalance_ratio < 0.3:
                validation['warnings'].append(f'Moderate class imbalance: {imbalance_ratio:.3f}')
        
        # Check for missing classes
        expected_classes = [-1, 0, 1]
        missing_classes = [cls for cls in expected_classes if cls not in label_counts.index]
        if missing_classes:
        ...
        if 'forward_return' in data.columns:
            ...
            flat_mask = (data['forward_return'] >= self.down_threshold) & (data['forward_return'] <= self.up_threshold)
            ...
            total_mismatches = up_label_mismatch + down_label_mismatch + flat_label_mismatch
            
            if total_mismatches > 0:
                validation['issues'].append(f'{total_mismatches} label-return mismatches found')
                validation['valid'] = False
        
        return validation
    
    def create_binary_labels(self, data: pd.DataFrame, target_class: int = 1) -> pd.DataFrame:
        """
    ...
    def get_feature_target_split(self, data: pd.DataFrame, feature_cols: list, 
                                target_col: str = 'label') -> Tuple[pd.DataFrame, pd.Series]:
        """
        ...
        logger.info(f"Created feature-target split: {len(features)} samples, {len(feature_cols)} features")
