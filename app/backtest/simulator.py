...
from app.utils.costs import calculate_trading_costs, calculate_position_sizing, calculate_sharpe_ratio, calculate_max_drawdown, calculate_profit_factor
...
class BacktestSimulator:
    ...
    def run_backtest(self, 
                    ...
                    down_threshold: float = 0.5) -> Dict[str, Any]:
        """
        ...
        for i in range(len(data)):
            ...
            current_volatility = self._estimate_volatility(data.iloc[max(0, i-60):i+1])
            ...
            if i < len(predictions):
                ...
                if pred == 1 and prob_up >= up_threshold:
                    self._open_position('long', current_time, current_price, current_volatility)
                
                # Short signal
                elif pred == -1 and prob_down >= down_threshold:
                    self._open_position('short', current_time, current_price, current_volatility)
    ...
    def _open_position(self, side: str, timestamp: datetime, price: float, volatility: float):
        """
        ...
        # Calculate costs
        costs = calculate_trading_costs(price, position_size, side, self.fee_bps, self.slippage_bps)
        
        # Check if we have enough capital
        required_capital = costs['notional'] + costs['total_cost']
        if required_capital > self.capital:
            logger.warning(f"Insufficient capital for {side} position: {required_capital:.2f} > {self.capital:.2f}")
            return
        ...
        logger.debug(f"Opened {side} position: {position_size:.6f} at {costs['effective_price']:.2f}")
    ...
    def _close_position(self, position: Dict, exit_time: datetime, exit_price: float):
        ...
        if position['side'] == 'long':
            gross_pnl = (exit_costs['effective_price'] - position['entry_price']) * position['size']
        else:
            gross_pnl = (position['entry_price'] - exit_costs['effective_price']) * position['size']
    ...
    def _calculate_metrics(self) -> Dict[str, Any]:
        ...
        profit_factor = calculate_profit_factor(
            np.array(winning_trades), 
            np.array([abs(pnl) for pnl in losing_trades])
        )
