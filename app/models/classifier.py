...
class XGBoostClassifier:
    ...
    def __init__(self, model_params: Dict[str, Any], artifacts_dir: str = "./app/artifacts"):
        if xgb is None:
            raise ImportError("XGBoost is required but not installed. Install with: uv add xgboost")
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, validation_split: float = 0.2) -> Dict[str, Any]:
        ...
        """
        logger.info(f"Training XGBoost classifier with {len(X)} samples and {len(X.columns)} features")
        ...
        dtrain = xgb.DMatrix(X_train, label=y_train_encoded, feature_names=self.feature_names)
        ...
        logger.info(f"Training completed - Train accuracy: {train_accuracy:.4f}, Val accuracy: {val_accuracy:.4f}")
    ...
    def cross_validate(self, X: pd.DataFrame, y: pd.Series, cv_folds: int = 5) -> Dict[str, Any]:
        ...
        logger.info(f"CV completed - Mean accuracy: {cv_metrics['cv_mean_accuracy']:.4f} ± {cv_metrics['cv_std_accuracy']:.4f}")
